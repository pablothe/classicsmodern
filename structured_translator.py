...
from book_validator import validate_book, ValidationReport
...
class BookParser:
    ...
    def parse(self, input_file: Path) -> BookStructure:
        ...
        detector = ChapterDetector(text, input_file.name)
        chapters_info = detector.detect_chapters_in_content()

        if not chapters_info:
            raise ValueError(f"No chapters detected in {input_file}")

        # Extract chapter content (text between markers)
        chapters = []
        lines = text.split('\n')
        ...
        for i, ch_info in enumerate(chapters_info):
            ...
            # Find end of this chapter (start of next or EOF)
            if i + 1 < len(chapters_info):
                end_line = chapters_info[i + 1]['line']
            else:
                end_line = len(lines)

            # Content is everything from marker line to next chapter
            # Skip the marker line itself (line after marker to end)
            ...
            chapters.append(Chapter(
                ...
                end_line=end_line,
                metadata={'type': ch_info['type'], 'line': ch_info['line']}
            ))
    ...
    def validate(self, structure: BookStructure) -> ValidationReport:
        ...
        if not report.valid:
            ...
            error_msg.extend([
                ...
                f"  python3 book_validator.py {structure.original_file} --auto-fix"
            ])
